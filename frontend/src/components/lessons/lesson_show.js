  ...
  handleSubmit(event) {
    ...
    if (this.state.text) {
      ...
      this.props
        ...
        .then(() => this.props.getLesson(this.props.match.params.id))
      ...
      return (
        <div className="lesson-show-page">
          <video className="video-tag" controls>
            <source src={this.props.lesson.videoUrl} type="video/mp4" />
          </video>
          {this.state.loading2 == false ? (
            <CommentHeatmap
              onHit={this.handleScroll}
              lesson={this.props.lesson}
              vidLength={this.vidLength}
            />
          ) : null}
          <div className="lesson-info">
            <p className="lesson-info-title">{this.props.lesson.title}</p>
            <p className="lesson-info-instructor">
              {this.props.lesson.instructor.firstName}{' '}
              {this.props.lesson.instructor.lastName}
            </p>
          ...
          <div className="comment-box" id="comment-box">
            {Object.values(this.props.lesson.comments).map((comment, idx) => (
              <Comment
                ...
                deleteComment={() => this.props.deleteComment(comment._id)}
